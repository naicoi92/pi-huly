...
export async function findPersonByEmailOrName(
  ...
  if (input.includes("@")) {
    ...
    // Self-email fallback: self-host Huly thường KHÔNG tạo Channel email cho
    // Person (verified: 0 email Channel trên workspace test). Link account→Person
    // thật sự là Account.uuid === Person.personUuid. Account KHÔNG queryable qua
    // findOne ("domain not found") + getAccount() KHÔNG expose email arbitrary,
    // nên chỉ resolve được KHI input === currentUser.email (self-assign — case
    // phổ biến nhất, vd update_issue assignee=<email get_user_profile trả>).
