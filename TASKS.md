...
**Status: ✅ implementation completed (prep) — release EXECUTION deferred to maintainer** _(slash goal complete-milestone M5, 2026-07-27 — 4/4 tasks done: T-36 e2e smoke integration harness + T-37 docs/NOTICE R1 audit + T-38 publish prep + T-39 release runbook; 365 tests, CI green, DoD pass for implementation; npm publish + git tag v1.0.0 KHÔNG tự động — outward-facing/irreversible, maintainer runs T-39 runbook §3)_
...
- [x] [T-39] [M] release (Bước 10: deployment strategy, git tag v1.0.0, post-deploy prod verify, monitoring window) — high | blocked-by: T-38 — ✅ prep done (PR #21 merged 3a3944b, runbook docs/design/plans/T-39-implement-plan.md: pre-release audit all gates pass + release commands + post-publish verify + rollback + monitoring; EXECUTION DEFERRED to maintainer — npm publish + git tag v1.0.0 KHÔNG tự động vì outward-facing + irreversible, user chưa confirm npm publish access)
