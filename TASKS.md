...
- [x] [T-64] [M] bug(client): gate "client websocket error" spam + token leak (URL `_transactor/<token>` ra stderr/UI) — high | blocked-by: T-62 | blocks: (none) | issue: #69 — [detail](./docs/tasks/T-64.md) — ✅ done (đăng ký 7 pattern vào DEFAULT_UPSTREAM_NOISE_PATTERNS T-62 framework: client websocket error + Generate SessionId + no ping + Connected + Processing upgrade + measure slow findAll; security guard: stderr captured KHÔNG chứa _transactor/ + token substring; Error instance KHÔNG filter (real error); WS onerror throw pathway vẫn propagate qua mapError; +6 tests = 578 total, CI green)
