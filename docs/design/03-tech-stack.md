...
| R7 | **pi-subagents process model UNVERIFIED** — D14 shared pool contingent on same-process; "fresh/forked context" = conversation/git isolation, chưa confirm same-process/module-instance | 🟡 | 🔴 | smoke test Bước 4: dispatch subagent gọi huly tool, confirm 1 connection (no reconnect). Fallback nếu break: per-subagent connect OR pre-connect + pass handle | maintainer | Bước 4 |
